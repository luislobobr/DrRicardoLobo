...
const PatientList = ({ patients, onSelectPatient, onNewPatient }) => {
  ...
  const [cityFilter, setCityFilter] = useState('Todos'); // Novo
  ...
  }, [patients, searchTerm, cityFilter]); // Atualizado
              ...
              <li key={patient.id}>
                <button onClick={() => onSelectPatient(patient)} className="w-full text-left p-4 hover:bg-gray-50 transition-colors duration-150 flex justify-between items-center">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-blue-700 truncate">{patient.name}</p>
...
export default function App() {
  ...
  const handleAddPatient = async (patientData, onSuccess = () => {}) => { // Modificado
    setIsSubmitting(true);
    try {
      // ... verificação de CPF removida ...
      
      const docRef = await addDoc(collection(db, patientsCollectionPath), { // Modificado
        ...patientData,
